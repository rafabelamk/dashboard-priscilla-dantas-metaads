...
async function fetchAccount(account, produto, sinceDate, untilDate, TOKEN, getAction, qs) {
  ...
  console.log(`[${account}] campaign HTTP status: ${campaignRes.status}`);
  const campaignData = await campaignRes.json();
  if (campaignData.error) {
    console.error(`[${account}] campaign error:`, JSON.stringify(campaignData.error));
    return null;
  }

  const campaign = campaignData.data?.find(c => c.name === CAMPAIGNS[produto]);
  if (!campaign) {
    console.log(`[${account}] campanha não encontrada para ${produto}. Disponíveis:`, campaignData.data?.map(c=>c.name).slice(0,3));
    return null;
  }
  console.log(`[${account}] campanha encontrada: ${campaign.name} (${campaign.id})`);
