...
async function fetchAccount(account, produto, sinceDate, untilDate, TOKEN, getAction, qs) {
  ...
  const isToday = sinceDate === today && untilDate === today;
  const timeParams = isToday
    ? { date_preset: 'today' }
    : { time_range: JSON.stringify({ since: sinceDate, until: untilDate }) };

  // 2. Insights diários
  const insightsRes = await fetch(`${BASE}/${campaign.id}/insights?${qs({
    fields: 'date_start,spend,impressions,inline_link_clicks,cpm,actions',
    ...timeParams,
    time_increment: '1',
    access_token: TOKEN,
    limit: '500',
  ...
  let insightsData;
  try { insightsData = JSON.parse(insightsText); } catch(e) { return null; }
  if (insightsData.error) return null;

  // 3. Insights por anúncio
  const adInsightsRes = await fetch(`${BASE}/${campaign.id}/insights?${qs({
    fields: 'ad_name,ad_id,spend,impressions,inline_link_clicks,cpm,actions',
    ...timeParams,
    level: 'ad',
    access_token: TOKEN,
    limit: '200',
  ...
  let adInsightsData;
  try { adInsightsData = JSON.parse(adInsightsText); } catch(e) { return null; }
  if (adInsightsData.error) return null;
