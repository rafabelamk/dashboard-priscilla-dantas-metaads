...
| Conta | ID | Status |
|---|---|---|
| Conta principal | `act_1519004369175892` | Pausada temporariamente |
| Conta ativa | `act_1457463461835190` | Ativa |

Ambas as contas estão na mesma BM e usam o mesmo `META_TOKEN`. O `api/meta.js` busca em paralelo e agrega os resultados automaticamente. Para ativar ou pausar uma conta, basta comentar/descomentar a linha correspondente no array `AD_ACCOUNTS`.
...
- **Campanhas KD26:** `[KD26][VENDAS][FRIO][ADV][CBO] Vendas Perpétuo`
- **Campanhas PPRR26:** `[PPRR26][VSL][VENDAS][HOT][ADV][CBO] Vendas Perpétuo`
...
├── api/
│   └── meta.js         # Serverless: busca e agrega insights de múltiplas contas Meta
...
```bash
git add index.html api/meta.js
